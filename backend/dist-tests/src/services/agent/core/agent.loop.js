 ...
 */
const emitAgentEvent = async (event, onEvent) => {
    if (!onEvent) {
        return;
    }
    await onEvent(event);
 ...
 * @returns 決策是否合法。
 */
const validateToolDecision = (decision, availableToolNames) => {
 ...
 */
const createToolContext = (toolName, toolInput, toolResult) => {
    if (toolResult.ok) {
...
export const runAgentLoop = async (input) => {
    const { disableTools = false, onEvent, userMessage, historyMessages = [], roleName = "primary", signal, skillName, } = input;
    ...
    for (let attempt = 0; attempt < maxLoops; attempt += 1) {
        ...
        const decision = await llmService.askAgentDecision(messages, signal);
