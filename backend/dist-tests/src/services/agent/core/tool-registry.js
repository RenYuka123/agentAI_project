...
const tools = [calculatorTool, summarizeTextTool, getStockPriceTool, getWeatherTool];
/**
 * 使用單一工具清單，確保 prompt 與實際執行器使用的是同一批工具。
 *
 * @returns 目前可用的工具清單。
 */
export const getToolList = (skillName, roleName) => {
    const skill = getSkillByName(skillName);
    const role = getAgentRoleByName(roleName || "primary");
    const roleScopedTools = role && Array.isArray(role.allowedTools)
 ...
 */
export const getToolByName = (name) => tools.find((tool) => tool.name === name);
