 ...
 * @returns 摘要句子陣列。
 */
const buildSummary = (text, maxSentences) => {
...
export const summarizeTextTool = {
    ...
    validateInput: (input) => {
        ...
        const maxSentences = typeof input.maxSentences === "number" && input.maxSentences > 0 ? Math.floor(input.maxSentences) : 3;
        if (!text) {
    ...
    async execute(input) {
        ...
        const summary = format === "paragraph"
            ? summaryParts.join(" ")
