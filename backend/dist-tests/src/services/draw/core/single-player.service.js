...
const parsePngDataUrl = (imageDataUrl) => {
...
const analyzeDrawing = (imageDataUrl) => {
    ...
    for (let index = 0; index < png.data.length; index += 16) {
        ...
        distinctColors.add(`${Math.round(red / 32)}-${Math.round(green / 32)}-${Math.round(blue / 32)}`);
    }
    /* 計算各項指標 */
    /* 覆蓋率 */
     ...
     *  */
    const qualityScore = Math.min(1, coverage * 3.2 + strokeScore * 0.25 + colorScore * 0.2);
    return {
        coverage,
...
const pickWrongGuess = (answer, seed) => {
...
const isGuessConfidence = (value) => {
    return value === "high" || value === "medium" || value === "low";
};
const buildVisionPrompt = (timedOut) => {
...
/* 向視覺模型請求猜測 */
const requestVisionGuess = async (imageDataUrl, answer, timedOut) => {
    if (!llmConfig.apiKey) {
        logger.warn("Single player vision is unavailable because LLM_API_KEY is missing.");
        return null;
    }
    ...
    if (!response.ok) {
        ...
        logger.warn("Single player vision request failed, falling back to heuristic mode.", {
    ...
    if (!outputText) {
        logger.warn("Single player vision returned empty output, falling back to heuristic mode.");
        return null;
    }
    const parsed = safeJsonParse(outputText);
    if (!parsed.ok || typeof parsed.data !== "object" || parsed.data === null) {
        logger.warn("Single player vision returned invalid JSON, falling back to heuristic mode.", {
            outputText,
        });
        return null;
    }
    ...
    const confidence = isGuessConfidence(payload.confidence)
        ...
        : "low";
    if (!guess) {
        logger.warn("Single player vision returned no guess, falling back to heuristic mode.", {
            outputText,
        });
        return null;
    }
...
/* 建立正面評論 */
const buildPositiveComment = (analysis, timedOut) => {
    if (timedOut) {
...
/* 建立錯誤評論 */
const buildMissComment = (analysis, timedOut) => {
    if (timedOut) {
...
/* 建立猜測結果 */
const buildHeuristicGuess = (analysis, answer, imageDataUrl, timedOut) => {
    if (analysis.isBlank) {
        return {
            guess: "???",
            comment: timedOut
    ...
    /* 成功機率 */
    const successChance = Math.max(0.12, Math.min(0.92, analysis.qualityScore + detailBonus - timeoutPenalty));
    /* 信心等級判定 (confidence)根據最終機率決定顯示給玩家看的「信心程度」 */
    const confidence = successChance > 0.72 ? "high" : successChance > 0.42 ? "medium" : "low";
    /* 用穩定 seed 取代真正的隨機，讓同一份圖片輸入能得到一致結果。 */
    const roll = createDeterministicRandom(`${answer}|${timedOut ? "timed-out" : "submitted"}|${imageDataUrl.slice(0, 256)}`);
...
export const singlePlayerService = {
    async guessDrawing(input) {
        const answer = input.answer.trim();
        if (!answer) {
            throw new Error("`answer` is required.");
        }
        const visionResult = await requestVisionGuess(input.imageDataUrl, answer, Boolean(input.timedOut));
        if (visionResult) {
            return {
                ...visionResult,
        ...
        const result = buildHeuristicGuess(analysis, answer, input.imageDataUrl, Boolean(input.timedOut));
        return {
            ...result,
