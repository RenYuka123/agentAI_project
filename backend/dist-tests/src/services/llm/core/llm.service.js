 ...
 * @returns 純工具結果字串。
 */
const extractToolResultContent = (message) => {
 ...
 */
const createFallbackFinalAnswer = (toolMessage) => {
    ...
    if (toolMessage.toolName === "get_stock_price") {
        ...
        return `${String(data.symbol ?? "標的")} 最新價格約為 ${String(data.price ?? "?")} ${String(data.currency ?? "")}。`.trim();
    }
    return toolResult;
};
/**
 ...
 */
const createLocalFallbackDecision = (messages) => {
...
const hasMissingApiKeyError = (error) => error instanceof Error && error.message.includes("LLM_API_KEY is not configured");
export const llmService = {
    async askAgentDecision(messages, signal) {
