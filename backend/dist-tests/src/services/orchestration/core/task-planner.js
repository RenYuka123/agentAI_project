...
const countMatches = (message, pattern) => {
 ...
 */
const estimateDependencyShape = (normalizedMessage, intentCount) => {
    ...
    const estimatedTaskCount = Math.max(1, Math.min(3, Math.max(intentCount, dependencyCueCount + parallelCueCount + 1)));
    const estimatedDependencyDepth = Math.max(1, Math.min(3, dependencyCueCount + (parallelCueCount > 0 ? 1 : 0) + 1));
    const isDependencyChainLikely = dependencyCueCount >= Math.max(1, parallelCueCount) && estimatedDependencyDepth >= 2;
    return {
        dependencyCueCount,
        parallelCueCount,
        estimatedTaskCount,
...
const createRuleBasedAssessment = (context) => {
    ...
    if (dependencyShape.isDependencyChainLikely) {
        ...
        reasons.push(`依賴 cue 偏強，預估 dependency 深度約 ${dependencyShape.estimatedDependencyDepth}，較像線性串接流程。`);
    }
...
const isOrchestrationAssessment = (value) => {
    if (!value || typeof value !== "object") {
        return false;
    }
    const candidate = value;
    return (typeof candidate.shouldOrchestrate === "boolean" &&
...
const buildRouterMessages = (context, ruleAssessment) => [
    {
        role: "system",
        content: [
...
export const assessOrchestration = async (input) => {
    ...
    try {
        const llmAssessment = await llmService.askJson({
 ...
 * 建立標準化的 worker 子任務資料。
 */
const buildTask = (taskId, title, instruction, dependsOn = []) => ({
    taskId,
    title,
    instruction,
    role: "worker",
 ...
 */
const normalizeTaskId = (value, index) => {
 ...
 */
export const sanitizeTaskPlan = (value) => {
    if (!value || typeof value !== "object") {
        return undefined;
    }
    const candidate = value;
    ...
    const tasks = rawTasks
        .map((task, index) => {
        const taskId = normalizedTaskIds[index];
        const dependsOn = Array.from(new Set((task.dependsOn ?? [])
 ...
 */
export const createFallbackTaskPlan = (context) => {
    const { skillName, userMessage } = context;
    const tasks = [];
    const normalizedMessage = userMessage.trim();
    if (skillName === "investment_analysis") {
        tasks.push(buildTask("collect-data", "蒐集投資資料", `針對這個投資請求先蒐集必要資料與數值，只回答與資料蒐集和必要計算直接相關的內容：${normalizedMessage}`));
        tasks.push(buildTask("summarize-analysis", "整理投資分析", `根據前一個子任務的資料，整理成簡潔的投資分析重點。如果缺乏資料，就明確指出限制。原始需求：${normalizedMessage}`, ["collect-data"]));
    }
    else if (skillName === "weather_summary") {
        tasks.push(buildTask("collect-weather", "蒐集天氣資料", `先取得完成這個天氣需求所需的資訊，只回答天氣事實與條件：${normalizedMessage}`));
        tasks.push(buildTask("summarize-weather", "整理天氣建議", `根據前一個子任務的天氣結果，整理成自然語言摘要與生活提醒。原始需求：${normalizedMessage}`, ["collect-weather"]));
    }
    else {
        tasks.push(buildTask("analyze-request", "處理前半段需求", `先完成這個需求中最前面的核心步驟，必要時使用工具，但只回答這一步的結果：${normalizedMessage}`));
        tasks.push(buildTask("compose-answer", "整理後半段需求", `根據前一個子任務結果，繼續完成剩餘需求並補上整體整理。原始需求：${normalizedMessage}`, ["analyze-request"]));
    }
    return {
        tasks,
