 ...
 */
const emitEvent = async (event, onEvent) => {
    if (!onEvent) {
        return;
    }
    await onEvent(event);
 ...
 */
const resolveExecutionOrder = (tasks) => {
    const taskMap = new Map(tasks.map((task) => [task.taskId, task]));
    const missingDependencies = tasks.flatMap((task) => (task.dependsOn ?? [])
 ...
 */
const getDependencyResults = (task, taskResults) => {
 ...
 */
const executeTask = async (task, input, dependencyResults) => {
