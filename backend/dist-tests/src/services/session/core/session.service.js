...
const ensureSession = async (sessionId) => {
    ...
    await sessionsCollection.updateOne({ sessionId: resolvedSessionId }, {
        $setOnInsert: {
 ...
 * @returns 是否應顯示在聊天畫面上。
 */
const isVisibleHistoryMessage = (record) => {
    if (record.role === "user") {
        return true;
    }
    if (record.role === "tool") {
 ...
 * @returns 已整理好的歷史訊息列表。
 */
const getVisibleMessages = async (sessionId) => {
    const messagesCollection = await getMessagesCollection();
 ...
 */
const ensureSessionTitle = async (sessionId, message) => {
    const sessionsCollection = await getSessionsCollection();
