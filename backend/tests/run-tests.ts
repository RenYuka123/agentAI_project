...
import type { AgentMessage } from "../src/services/agent/index.js";
...
const createMockSseResponse = () => {
  ...
  res.status = (_code: number) => res;
  res.json = (_payload: unknown) => res;
  ...
  },
  {
    ...
    run: async () => {
      ...
      try {
        ...
        assert.ok(
          parallelAssessment.score > linearAssessment.score,
          `Expected parallel score (${parallelAssessment.score}) to be greater than linear score (${linearAssessment.score}).`,
        );
  ...
  },
  {
    ...
    run: async () => {
      ...
      try {
        ...
        const streamPromise = chatStream(req, res as any);
