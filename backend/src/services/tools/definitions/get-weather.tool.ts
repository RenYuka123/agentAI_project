...
import type { AgentTool, ToolHealthStatus } from "../core/tool.types.js";

interface GetWeatherToolInput extends JsonObject {
  /** 要查詢的地點名稱。 */
  location: string;
  /** 可選的國碼篩選。 */
...
interface NormalizedWeatherLocation {
  /** 正規化後的查詢地點名稱。 */
  location: string;
  /** 推導後的國碼。 */
  countryCode: string | null;
 ...
 * @returns 正規化後的地點與國碼。
 */
const normalizeWeatherLocation = (location: string, countryCode: string | null): NormalizedWeatherLocation => {
  const trimmedLocation = location.trim();
  const normalizedChineseLocation = trimmedLocation.replace(/台/g, "臺");
  const aliasLocation =
    taiwanLocationAliases[trimmedLocation] ??
 ...
 */
const getWeatherCodeDescription = (weatherCode?: number): string => {
 ...
 * @returns 工具健康狀態。
 */
const checkWeatherToolHealth = async (): Promise<ToolHealthStatus> => {
  try {
 ...
 */
export const getWeatherTool: AgentTool<GetWeatherToolInput, JsonObject> = {
  name: "get_weather",
