...
interface StockPriceToolInput extends JsonObject {
  /** 要查詢的股票或 ETF 代號。 */
  symbol: string;
}
 ...
 * @returns 查詢月份的第一天字串。
 */
const formatTwseMonthDate = (date: Date): string => {
 ...
 * @returns 可用數字或 null。
 */
const parseTwseNumber = (value: string | undefined): number | null => {
  if (!value) {
    return null;
  }
 ...
 */
const extractLatestStockPrice = (payload: TwseStockDayResponse, symbol: string): StockPriceApiResponse | null => {
...
export const getStockPriceTool: AgentTool<StockPriceToolInput, JsonObject> = {
  ...
  async execute(input) {
    ...
    const successResult: JsonObject = {
      tool: "get_stock_price",
