...
import type { AgentSkillSelection } from "../../agent/index.js";
 ...
 */
const buildRoutingContext = (historyMessages: AgentMessage[], userMessage: string): string =>
  [
 ...
 */
const inferSkillFromHistory = (historyMessages: AgentMessage[]): string | undefined => {
  const recentMessages = historyMessages.slice(-10);

  const usedWeatherTool = recentMessages.some(
    (message) => message.role === "tool" && message.toolName === "get_weather",
  );
  if (usedWeatherTool) {
...
export const resolveSkillSelection = (input: {
  ...
  const matchedSkills = getSkillList()
    .map((skill) => {
      const matchedHints = (skill.routingHints ?? []).filter((hint) =>
        routingContext.includes(hint.toLowerCase()),
      );
